class HyperLogLog {
  constructor(size) {
    this.size = size;
    this.bucketCount = Math.pow(2, size);
  ...
  merge(data) {
    if (this.size > data.size) {
      let newBucketCount = Math.pow(2, data.size);
      let oldBucketsPerNew = Math.pow(2, this.size - data.size);
    ...
    for (let i = 0; i < this.bucketCount; ++i) {
      ...
      this.sumOfInverses += Math.pow(2, -bucket);
    }
  }
}
